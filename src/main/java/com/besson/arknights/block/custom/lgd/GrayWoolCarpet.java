...
public class GrayWoolCarpet extends FurnitureHorizontalFacingBlock {
    ...
    public GrayWoolCarpet(Settings settings) {
        super(settings);
    }
    ...
    public void onPlaced(World world, BlockPos pos, BlockState state, @Nullable LivingEntity placer, ItemStack itemStack) {
        ...
        world.setBlockState(right, ModBlocks.GRAY_WOOL_CARPET_RIGHT.getDefaultState().with(FACING, dir));
    }
    ...
    private BlockPos findMainPos(BlockState state, BlockPos pos){
        if (state.isOf(ModBlocks.GRAY_WOOL_CARPET_LEFT)){
            return pos;
        }
        Direction dir = state.get(FACING);
        if (state.isOf(ModBlocks.GRAY_WOOL_CARPET_RIGHT)){
    ...
    private boolean isBookcase(BlockState state){
        return state.isOf(this) ||
                state.isOf(ModBlocks.GRAY_WOOL_CARPET_LEFT) ||
                state.isOf(ModBlocks.GRAY_WOOL_CARPET_RIGHT);
    }
}
