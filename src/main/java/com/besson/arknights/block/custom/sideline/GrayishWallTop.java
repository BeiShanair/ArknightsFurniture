...
public class GrayishWallTop extends FurnitureHorizontalFacingBlock {
    private static final VoxelShape SHAPE = Block.createCuboidShape(0, 0, 0, 16, 16, 16);

    public GrayishWallTop(Settings settings) {
        super(settings);
    }
    ...
    public void onPlaced(World world, BlockPos pos, BlockState state, @Nullable LivingEntity placer, ItemStack itemStack) {
        ...
        world.setBlockState(bbr, ModBlocks.GRAYISH_WALL_TBR.getDefaultState().with(FACING, dir));
        world.setBlockState(btl, ModBlocks.GRAYISH_WALL_TTL.getDefaultState().with(FACING, dir));
        world.setBlockState(btr, ModBlocks.GRAYISH_WALL_TTR.getDefaultState().with(FACING, dir));
    }
    ...
    private void removeAllParts(World world, BlockPos mainPos, Direction dir) {
        ...
        for (BlockPos partPos : partPositions) {
            if (isWallBottom(world.getBlockState(partPos))) {
    ...
    private BlockPos findMainBlockPosition(BlockState state, BlockPos pos) {
        // If current block is the main block (bottom-left)
        if (state.isOf(ModBlocks.GRAYISH_WALL_TBL)) {
            return pos;
        }
        Direction dir = state.get(FACING);
        if (state.isOf(ModBlocks.GRAYISH_WALL_TBR)){
            return pos.offset(dir.rotateYClockwise());
        } else if (state.isOf(ModBlocks.GRAYISH_WALL_TTL)){
            return pos.down();
        } else if (state.isOf(ModBlocks.GRAYISH_WALL_TTR)) {
    ...
    private boolean isWallBottom(BlockState state){
        return state.isOf(this) ||
                state.isOf(ModBlocks.GRAYISH_WALL_TBL) ||
                state.isOf(ModBlocks.GRAYISH_WALL_TBR) ||
                state.isOf(ModBlocks.GRAYISH_WALL_TTL) ||
                state.isOf(ModBlocks.GRAYISH_WALL_TTR);
    }
}
